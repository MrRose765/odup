...
def test_env_pull_workflow_normalizes_version(monkeypatch) -> None:
    captured: dict[str, str | None] = {"version": None}

    monkeypatch.setattr(workflows, "parse_version", lambda raw: "saas-19.2")

    def _fake_pull_existing_sources(version=None):
        captured["version"] = version
        return ["ok"], []

    monkeypatch.setattr(workflows, "pull_existing_sources", _fake_pull_existing_sources)
...
def test_env_pull_workflow_without_version(monkeypatch) -> None:
